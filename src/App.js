...
// --- FIREBASE AYARLARI (Sizin Verdiğiniz Bilgiler) ---
...
const appId = "uzem-takip-prod-v1"; // Veritabanı kök yolu
...
const ROLES = {
  EDUCATION: {
    ...
    name: "Eğitim Takip",
    pass: "egitim",
    access: ["dashboard", "education", "filming", "editing", "calendar"],
  },
  FILMING: {
    id: "filming",
    name: "Çekim Takip",
    pass: "c1t2",
    access: ["filming", "calendar"],
  },
  EDITING: {
    id: "editing",
    name: "Montaj Takip",
...
const LOOKUPS = {
  ...
  ALAN: [
    ...
    "Yönetim ve Hizmet",
    "Ziraat",
    "Ahşap Tasarımı ve Teknolojileri",
    "Süsleme Sanatları",
  ...
  ICERIK_TAKIP: [
    "Arzu Mantar",
    "Meltem Ermez",
    "Meltem Ermez - Nezahat Kara",
    "Nezahat Kara",
    "Nezahat Kara - Meltem Ermez",
    "Sevim Aydın Verim",
    "Sevim Aydın Verim - Meltem Ermez - Nezahat Kara",
  ],
  DURUM_GENEL: [
    "Eğitim Planlanıyor",
    "Eğitmen İçerik Hazırlıyor",
    "Ekran Çekiminde",
    "Etkileşimli İçerik Hazırlanıyor",
    "Çekim Bekliyor",
    "Çekimde",
    "Çekim Bitti",
    "Çekim Revize",
    "İçerik Bitti",
    "Ses Çekimi Bekleniyor",
    "İçerik Kontrolü",
    "İçerik Revize",
    "Montaj Sırasında",
    "Etkileşimli İçerik Sırasında",
    "Montajda",
    "Montaj Kontrolü",
    "Montaj Revize",
    "ID Bekliyor",
  ...
  MONTAJ_DURUMU: ["Başladı", "Devam Ediyor", "1.Revize", "2.Revize", "Bitti"],
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          {/* İSTEK: Başlık Değişikliği */}
          <h1 className="text-2xl font-bold text-gray-800">
            UZEM Eğitim Takip Sistemi
          </h1>
      ...
      <div className="p-4 border-b border-gray-200 flex flex-wrap items-center justify-between gap-4 bg-gray-50 rounded-t-xl">
        ...
        <div className="flex items-center gap-3 flex-1 max-w-3xl justify-end">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-md transition-colors"
          >
          ...
          <thead className="bg-gray-100 sticky top-0 z-10 shadow-sm text-gray-700">
            <tr>
              ...
              {columns.map((col) => (
                <th
                  key={col.key}
                  className="p-3 border-b font-semibold min-w-[120px]"
                >
                  {col.label}
                </th>
              ))}
              <th className="p-3 border-b font-semibold w-24 text-center bg-gray-100 sticky right-0 shadow-l">
                İşlemler
              </th>
            </tr>
          </thead>
            ...
            {isAdding && (
              <tr className="bg-indigo-50">
                <td className="p-3 text-center">
                  <Plus size={16} className="mx-auto text-indigo-500" />
                </td>
                {columns.map((col) => (
                ...
                >
                  ...
                  {columns.map((col) => (
                    <td key={col.key} className="p-3">
                      {isRowEditing
                        ? renderInput(col, editForm, setEditForm)
                        : col.type === "date"
...
const EducationPage = ({ currentUser }) => {
  ...
  const cols = [
    ...
    { key: "egitmen", label: "Eğitmen" },
    {
      ...
      label: "İçerik Takip",
      type: "select",
      options: LOOKUPS.ICERIK_TAKIP,
    },
    {
      key: "durum",
      label: "Durum",
      type: "select",
      options: LOOKUPS.DURUM_GENEL,
    },
    ...
    {
      key: "montajSorumlusu",
      label: "Montaj Sorumlusu",
      type: "select",
      options: LOOKUPS.MONTAJ_SORUMLUSU,
    },
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        // KURAL: Yayında olanlar yeşil ve en altta
        list.sort((a, b) => {
  ...
  const handleUpdate = async (id, form) => {
    await updateDoc(
      doc(db, "artifacts", appId, "public", "data", "education_tracking", id),
      form
    );
  };
  ...
  return (
    <DataTable
      ...
      customRowClass={(row) =>
        row.durum === "Yayında" ? "bg-green-50 border-green-200" : ""
      }
    />
  );
};
...
const FilmingPage = ({ currentUser }) => {
  ...
  const cols = [
    ...
    {
      key: "cekimSorumlusu",
      label: "Çekim Sorumlusu",
      type: "select",
      options: LOOKUPS.CEKIM_SORUMLUSU,
    },
    {
      key: "isikSorumlusu",
      label: "Işık Sorumlusu",
      type: "select",
      options: LOOKUPS.CEKIM_SORUMLUSU,
    },
    { key: "videoAdi", label: "Video Adı" },
    { key: "cekimBaslama", label: "Çekim Başlama", type: "date" },
    {
      key: "onCekim",
      label: "Ön Çekim",
      type: "select",
      options: LOOKUPS.EVET_HAYIR,
    },
    { key: "onCekimTarihi", label: "Ön Çekim Tarihi", type: "date" },
    {
      key: "izlence",
      label: "İzlence (İçerik Uzm.)",
      type: "select",
      options: LOOKUPS.ICERIK_UZMANI,
    },
    {
      key: "cekimDurumu",
      label: "Çekim Durumu",
      type: "select",
      options: LOOKUPS.CEKIM_DURUMU,
    },
    ...
    {
      key: "cekimKontrolYapan",
      label: "Çekim Kontrol Yapan",
      type: "select",
      options: LOOKUPS.CEKIM_SORUMLUSU,
    },
    {
    ...
    },
    {
      ...
      label: "Çekim",
      type: "select",
      options: LOOKUPS.TAMAMLANDI_DURUMU,
    },
    {
      key: "synology",
      label: "Synology",
      type: "select",
      options: LOOKUPS.SYNOLOGY_DURUMU,
    },
    { key: "synologyKlasor", label: "Synology Klasör Adı" },
    { key: "synologyVideoAdi", label: "Video Synology Adı (Kod)" },
    {
      key: "cekimYapanlar",
      label: "Çekim Yapanlar",
      type: "select",
      options: LOOKUPS.CEKIM_SORUMLUSU,
    },
  ...
  useEffect(() => {
    if (!currentUser) return;
    const q = query(
      collection(db, "artifacts", appId, "public", "data", "filming_tracking"),
      orderBy("createdAt", "desc")
    );
    const unsub = onSnapshot(
      q,
      (snap) => {
        let list = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        // KURAL: Çekim Durumu Bitti olanlar yeşil ve en altta
        list.sort((a, b) => {
  ...
  const handleUpdate = async (id, form) => {
    await updateDoc(
      doc(db, "artifacts", appId, "public", "data", "filming_tracking", id),
      form
    );
  };
  const handleDelete = async (id) => {
  ...
  return (
    <DataTable
      ...
      onDelete={handleDelete}
      customRowClass={(row) =>
        row.cekimDurumu === "Bitti" ? "bg-green-50 border-green-200" : ""
      }
    />
  );
};
...
const EditingPage = ({ currentUser }) => {
  ...
  const cols = [
    ...
    { key: "bitenVideo", label: "Biten Video Adı" },
    {
      key: "icerikUzmani",
      label: "İçerik Uzmanı",
      type: "select",
  ...
  useEffect(() => {
    if (!currentUser) return;
    const q = query(
      collection(db, "artifacts", appId, "public", "data", "editing_tracking"),
      orderBy("createdAt", "desc")
    );
    const unsub = onSnapshot(
      q,
      (snap) => {
        let list = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        // KURAL: Montaj Durumu Bitti olanlar yeşil ve en altta
        list.sort((a, b) => {
          if (a.montajDurumu === "Bitti" && b.montajDurumu !== "Bitti")
            return 1;
          if (a.montajDurumu !== "Bitti" && b.montajDurumu === "Bitti")
            return -1;
          return 0;
  ...
  const handleUpdate = async (id, form) => {
    await updateDoc(
      doc(db, "artifacts", appId, "public", "data", "editing_tracking", id),
      form
    );
  };
  const handleDelete = async (id) => {
  ...
  return (
    <DataTable
      ...
      onDelete={handleDelete}
      customRowClass={(row) =>
        row.montajDurumu === "Bitti" ? "bg-green-50 border-green-200" : ""
      }
    />
  );
};
...
const DashboardPage = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const unsub1 = onSnapshot(
      ...
      (s) => setEduData(s.docs.map((d) => d.data())),
      (e) => console.log("Dash Error 1", e)
    );
    const unsub2 = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "filming_tracking"),
      (s) => setFilmData(s.docs.map((d) => d.data())),
      (e) => console.log("Dash Error 2", e)
    );
    const unsub3 = onSnapshot(
      collection(db, "artifacts", appId, "public", "data", "editing_tracking"),
      (s) => setEditData(s.docs.map((d) => d.data())),
      (e) => console.log("Dash Error 3", e)
    );
    return () => {
      unsub1();
      unsub2();
  ...
  const inFilming = filmData.filter(
    (d) => d.cekimDurumu === "Devam Ediyor" || d.cekimDurumu === "Başladı"
  ).length;
  const inEditing = editData.filter(
    (d) =>
      d.montajDurumu === "Başladı" ||
      d.montajDurumu === "Devam Ediyor" ||
      d.montajDurumu?.includes("Revize")
  ).length;
  const published = eduData.filter((d) => d.durum === "Yayında").length;
  const totalEdu = eduData.length;
  ...
  const barData = {
    labels: ["Çekimde", "Montajda", "Yayında", "Toplam"],
    datasets: [
      {
        label: "Süreç Dağılımı",
        data: [inFilming, inEditing, published, totalEdu],
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initAuth = async () => {
      try {
        ...
        if (isMounted)
          setAuthError("Bağlantı hatası. Lütfen internetinizi kontrol edin.");
      }
    };
  ...
  const allowedMenu = menuItems.filter((item) =>
    userRole.access.includes(item.id)
  );
      ...
      <div className="w-64 bg-slate-900 text-white flex flex-col flex-shrink-0 shadow-xl z-20">
        <div className="p-6 border-b border-slate-800">
          ...
          <h1 className="text-xl font-bold tracking-tight">
            Eİİ <span className="text-indigo-400">UZEM</span>
          </h1>
          <p className="text-xs text-slate-400 mt-1">Prodüksiyon Paneli</p>
        </div>
        ...
        <div className="p-4 border-t border-slate-800">
          <div className="bg-slate-800 rounded-lg p-3 mb-3">
            ...
            <div className="font-bold text-sm truncate" title={userRole.name}>
              {userRole.name}
            </div>
          </div>
          <button
        ...
        <main className="flex-1 p-6 overflow-hidden flex flex-col">
          {activeTab === "dashboard" && (
            <div className="h-full overflow-y-auto custom-scrollbar">
              <DashboardPage currentUser={firebaseUser} />
            </div>
          )}
          {activeTab === "education" && (
            <EducationPage currentUser={firebaseUser} />
          )}
          {activeTab === "filming" && (
            <FilmingPage currentUser={firebaseUser} />
          )}
          {activeTab === "editing" && (
            <EditingPage currentUser={firebaseUser} />
          )}
